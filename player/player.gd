...
#mermi ateşlenirken yüklenecek olan sahne
...
#animatedsprite2dyi area2dye çevirdiğimiz için hata vermemesi için
...
func process_movement_input():
	...
	#çaprazda hızlı gitme sorunu
	velocity = velocity.normalized()

func direction_follows_input():
	if is_shooting==true:
		return
	#sola giderken karakterin dönmesi
	if velocity.x>0:
		sprite.flip_h=false
	elif velocity.x<0:
		sprite.flip_h=true		
